...
void Assembler::createMarkerGraphVerticesThreadFunction1Gpu(size_t threadId)
{
    ...
    if (getNextBatch(begin, end)) {
        ...
        for (size_t first=begin; first<end; first+=gpuBatchSize) {
            ...
            shasta_alignBatchGPU (maxMarkerFrequency, maxSkip, maxDrift, alignCount, numPos, numReads, batch_rid_marker_pos, batch_read_pairs, h_alignments, h_num_traceback, true);
