...
)
{
    ...
    for(uint64_t clusterId=0; clusterId<clusters.size(); clusterId++) {
        ...
        for(const vector<vertex_descriptor>& path: paths) {
            ...
            for(const auto& x: pathSetMap) {
                for(const auto& y: x.second) {
                    cout << x.first << " " << y << " " << y.lower() << " " << y.upper() << endl;
                }
            }

            // With this information we can construct the AssemblyGraphJourneyInterval for the new vertex.
        }
    }
}
